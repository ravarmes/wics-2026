"""
Grid search 2: configs D, E, F — exploracao de regularizacao e LR.

Motivacao:
  O grid anterior (train_grid_3e5.py) mostrou que o eval_loss sobe enquanto a
  acuracia estagna -> overfitting. Mais epocas pioram. Estas configs atacam o
  problema com regularizacao e fecham a lacuna do baseline 5e-5 no corpus atual.

Configs testadas:
  D: lr=5e-5, warmup=500, epochs=5, scheduler linear      (ancora 5e-5 fresca)
  E: lr=3e-5, warmup=500, epochs=5, cosine + label_smoothing=0.1
  F: lr=4e-5, warmup=500, epochs=5, scheduler linear      (LR intermediario)

Metodologia identica ao train_grid_3e5.py / train_sentiment_melhorado.py:
  - 5-fold CV estratificado nos 80% de treino
  - Oversampling aleatorio ("SMOTE") por fold
  - Holdout 20% avaliado apenas ao final, com o melhor fold de cada config

Nota tecnica (configs com parametros extras):
  get_training_args() em utils/training_utils.py tem assinatura fixa e nao
  aceita lr_scheduler_type nem label_smoothing_factor. Este script ESTENDE esse
  metodo em tempo de execucao (monkey-patch local) — nenhum arquivo do projeto e
  modificado. O patch apenas adiciona capacidade: para D e F (sem extras) o
  comportamento e identico ao original.

Saida:
  src/_baselines/grid_def_results.json  (nao sobrescreve nenhum resultado existente)
  Modelos em: src/app/nlp/models/trained/AS_grid_<X>_fold_<N>_<timestamp>/

Execucao:
  cd <projeto>/src
  python -m app.nlp.training.train_grid_def
"""
...
TRAINING_CONFIG['grid_D'] = {
    **_BASE, 'learning_rate': 5e-5, 'num_train_epochs': 5, 'warmup_steps': 500,
}
TRAINING_CONFIG['grid_E'] = {
    **_BASE, 'learning_rate': 3e-5, 'num_train_epochs': 5, 'warmup_steps': 500,
    'lr_scheduler_type': 'cosine', 'label_smoothing_factor': 0.1,
}
TRAINING_CONFIG['grid_F'] = {
    **_BASE, 'learning_rate': 4e-5, 'num_train_epochs': 5, 'warmup_steps': 500,
}

# ── estender get_training_args para aceitar parametros extras ──────────────────
# get_training_args() tem assinatura fixa; chaves nao previstas causariam
# TypeError. O patch abaixo extrai as chaves extras, deixa o original construir
# o TrainingArguments e aplica os extras via setattr (lidos pelo Trainer no
# .train()). Nada e gravado em disco — patch valido apenas neste processo.
from app.nlp.utils.training_utils import TrainingHelper

_ORIG_GET_TRAINING_ARGS = TrainingHelper.get_training_args

_STD_KEYS = {
    'num_train_epochs', 'per_device_train_batch_size', 'per_device_eval_batch_size',
    'learning_rate', 'warmup_steps', 'weight_decay', 'logging_steps', 'eval_steps',
    'save_steps', 'evaluation_strategy', 'save_strategy', 'load_best_model_at_end',
    'metric_for_best_model', 'greater_is_better',
}


def _patched_get_training_args(self, output_dir, **kwargs):
    extra = {k: kwargs.pop(k) for k in list(kwargs) if k not in _STD_KEYS}
    args = _ORIG_GET_TRAINING_ARGS(self, output_dir, **kwargs)
    for key, value in extra.items():
        setattr(args, key, value)
    if extra:
        logger.info(f"TrainingArguments estendido com: {extra}")
    return args


TrainingHelper.get_training_args = _patched_get_training_args
...
OUT_FILE = Path(project_root) / '_baselines' / 'grid_def_results.json'

# Modelo de referencia (holdout do corpus historico, lr=5e-5/wr500/ep5)
REFERENCIA = {
    'name': 'Referencia historica: lr=5e-5, warmup=500, epochs=5',
    'holdout': {
        'accuracy': 0.7727,
        'f1_macro': 0.7749,
        'per_class': {
...
# Melhor config do grid anterior (3e-5), no corpus ATUAL — para comparacao direta
GRID_3E5_B = {
    'name': 'Grid B (3e-5 / wr500 / ep5) — melhor do grid anterior',
    'holdout': {
        'accuracy': 0.7545,
        'f1_macro': 0.7598,
        'per_class': {
...
GRID = [
    {'name': 'D', 'config_name': 'grid_D', 'lr': 5e-5, 'warmup': 500,
     'epochs': 5, 'extra': 'linear'},
    {'name': 'E', 'config_name': 'grid_E', 'lr': 3e-5, 'warmup': 500,
     'epochs': 5, 'extra': 'cosine + LS=0.1'},
    {'name': 'F', 'config_name': 'grid_F', 'lr': 4e-5, 'warmup': 500,
     'epochs': 5, 'extra': 'linear'},
]
...
def apply_smote(train_texts: list, train_labels: list) -> tuple[list, list]:
    """Identico ao apply_smote de train_grid_3e5.py (oversampling aleatorio)."""
...
def eval_holdout(model_path: str,
    ...
    """Carrega o modelo salvo e avalia no holdout (mesmo metodo do run_all_02.py)."""
    import torch
...
def print_summary(all_results: list[dict]) -> None:
    ...
    print("=" * 96)
    header = (f"{'Config':<44} {'Acc Holdout':>11} {'F1 Macro':>9}"
              f" {'F1 Neg':>7} {'F1 Neu':>7} {'F1 Pos':>7} {'Val F1 med+-std':>16}")
    print(header)
    print("-" * 96)

    def _ref_line(label: str, ref: dict, tail: str) -> None:
        h  = ref['holdout']
        pc = h['per_class']
        print(f"{label:<44}"
    ...
    _ref_line('Referencia historica (5e-5/wr500/ep5)',
              REFERENCIA, '(corpus antigo)')
    _ref_line('Grid B (3e-5/wr500/ep5)',
              GRID_3E5_B, '(corpus atual)')
    print("-" * 96)
    ...
    for r in all_results:
        ...
        h  = r['holdout']
        pc = h['per_class']
        label = (f"Config {c['name']} (lr={c['lr']}/wr={c['warmup']}"
                 f"/ep={c['epochs']}, {c['extra']})")
